---
title: "리액트로 chrome extention 만들기 (setting)"
date: 2024-02-04
description: "chrome extention을 만들어 보자!"
thumbnail: "/post/5/cover.png"
---
...
좀 더 가볍고 크롬 기반의 하드웨어를 제어하는 프로그램은 없을까 고민하다 chrome extention으로 탭을 제어하는 방법이 있다는 것을 알게되었다. 설치도 가볍고, 업데이트도 쉽게 가능 하기 때문에 이번 기회에 react와 typescript를 이용하여 chrome extension을 만들어 보았다.
...
chrome extention의 핵심이라고 할 수 있는 파일인 `manifest.json` 은 브라우저에 이 앱에 대한 정보를 알려주는 파일이다. 이 앱이 크롬 익스텐션이라고 브라우저에 명시를 해줘야, extension 프로젝트를 만들 수 있다.
...
1. [chrome://extensions/](https://www.notion.so/chrome-extention-setting-52c0201962a44edbb51aa90916929229?pvs=21) 으로 들어가 우측 상단에 "개발자 모드"를 활성화 해준다.
2. "압축해제된 확장 프로그램을 로드합니다." 를 클릭하여 빌드 dist 폴더를 선택한다.
3. 추가된 크롬 extention을 확인하고, 핀 고정을 한다. 그리고 클릭하면 default_pop에 설정한 index.html 이 보인다.
